...
// =========================================================================
// 🎨 INICIALIZADOR DE TEMA E CONFIGURAÇÕES DINÂMICAS DO SaaS
// =========================================================================
...
function fazerLogin() {
  ...
  auth.signInWithEmailAndPassword(email, pass).catch(e => {
    console.error("Motivo do erro de login:", e);
...
const DB = {
  ...
  set: (k, v) => {
    MEM[k] = v;
    if(auth.currentUser) {
      fbDB.collection('dados_cliente').doc(k).set({ lista: v }).catch(e => {
    ...
    fbDB.collection('dados_cliente').doc(k).onSnapshot((docSnap) => {
      if(docSnap.exists) {
          ...
          if(k === K.col) {
              ...
              if(auth.currentUser) fbDB.collection('dados_cliente').doc(k).set({ lista: MEM[k] });
          } else {
              MEM[k] = [];
          }
      }
...
function buildDash(){
  ...
  if(document.getElementById('ch2')) {
      CHS['ch2']=new Chart(document.getElementById('ch2').getContext('2d'),{type:'pie',data:{labels:Object.keys(cats),datasets:[{data:Object.values(cats),backgroundColor:[APP_CONFIG.tema.p, APP_CONFIG.tema.pd, APP_CONFIG.tema.pl, APP_CONFIG.tema.bor, '#c9a0e0','#3f51b5']}]},options:{responsive:true,maintainAspectRatio:false}});
  }
}
...
function renderConc(){
  ...
  document.getElementById('tbc').innerHTML=d.length?d.map(c=>`<tr><td>${fd(c.dataConc)}</td><td><strong>${c.cliente}</strong></td><td>${(c.desc||'').slice(0,55)}</td><td style="color:var(--ok);font-weight:700">${brl(c.valor)}</td>
...
function salvarE(){
  ...
  const es=DB.get(K.e)||[];
  es.unshift({id:uid(),nome:n,categoria:gv('ecat'),qtd:Number(gv('eq')||0),min:Number(gv('em')||0),custo:Number(gv('ec')||0),preco:Number(gv('ep2')||0), sku:gv('esku').trim()});
...
function renderCl(){
  ...
  document.getElementById('tbcl').innerHTML=d.length?d.map(c=>{
    ...
    let telHtml = num.length >= 10 
      ? `<a href="https://wa.me/55${num}" target="_blank" style="color:#25D366; font-weight:bold; text-decoration:none;" title="Chamar no WhatsApp">💬 ${c.tel}</a>` 
      : (c.tel || '-');
...
function gerarR(reset = true){
  ...
  const fat=vs.reduce((a,b)=>a+Number(b.total),0); 
  ...
  const sEl=document.getElementById('rrs'); sEl.textContent=brl(saldoFinal);
  sEl.style.color=saldoFinal>=0?'var(--ok)':'var(--no)';
...
// ── 11. SISTEMA DE CÓDIGO DE BARRAS E CUPOM ──
function imprimirCupom(pedId) {
  ...
  let total = Number(p.valor || 0);
  let sinal = Number(p.sinal || 0);
  let rest = total - sinal;

  let w = window.open('', '_blank', 'width=400,height=600');
  w.document.write(`
    <html><head><title>Cupom - ${p.cliente}</title>
    <style>
      body { font-family: 'Courier New', Courier, monospace; width: 280px; margin: 0 auto; padding: 10px 0; color: #000; font-size: 14px; }
      .ctr { text-align: center; }
      .b { font-weight: bold; }
      .lin { border-bottom: 1px dashed #000; margin: 10px 0; }
      ...
      <div class="ctr">
        <h2 style="margin:0; font-size: 1.2rem;">${APP_CONFIG.nome_sistema}</h2>
        <p style="margin:5px 0; font-size: 0.85rem;">Documento Auxiliar - Não fiscal</p>
      </div>
      <div class="lin"></div>
      <p class="b">CLIENTE: ${p.cliente}</p>
      <p>WhatsApp: ${p.tel || 'Não informado'}</p>
      <p>Data Entrega: ${p.entrega ? fd(p.entrega) : 'A combinar'}</p>
      <div class="lin"></div>
      <p class="b" style="margin-bottom:8px;">DESCRIÇÃO DO PEDIDO:</p>
      <p style="font-size: 0.9rem;">${p.desc.replace(/\n/g, '<br>')}</p>
      <div class="lin"></div>
      <div class="row"><span>Total Bruto:</span><span class="b">${brl(total)}</span></div>
      <div class="row"><span>Sinal Pago:</span><span>${brl(sinal)}</span></div>
      <div class="row" style="font-size: 1.1rem; margin-top: 8px;"><span>Restante:</span><span class="b">${brl(rest)}</span></div>
      <div class="lin"></div>
      <div class="ctr b" style="font-size:1.3rem; margin: 15px 0;">
        ${p.pago ? '*** PAGO ***' : '*** PENDENTE ***'}
      </div>
      <div class="lin"></div>
      <div class="ctr" style="font-size:0.85rem; margin-top:15px;">
...
function gerarSKU() {
  document.getElementById('esku').value = Math.floor(Math.random() * 900000000000) + 100000000000;
}

function biparProduto(e) {
  if (e.key === 'Enter' || e.keyCode === 13) {
    e.preventDefault(); 
    let skuBipado = document.getElementById('vsku').value.trim();
    if (!skuBipado) return;

    let prod = (DB.get(K.e) || []).find(x => x.sku === skuBipado);
    if (prod) {
      document.getElementById('vp').value = prod.nome;
      document.getElementById('vu').value = prod.preco;
      document.getElementById('vcat').value = prod.categoria || 'Outros';
      document.getElementById('vq').value = '1';
      calcV(); 
      toast('✅ Produto identificado!');
      document.getElementById('vq').focus(); 
    } else {
      toast('❌ Código não encontrado no estoque!', true);
    }
...
function abrirCamera() {
  ...
  if (divLeitor.style.display === 'block') {
    if (leitorCamera) {
  ...
  leitorCamera.start(
    ...
    (codigoLido) => {
      document.getElementById('vsku').value = codigoLido;
      leitorCamera.stop().then(() => {
        divLeitor.style.display = 'none';
        biparProduto({ key: 'Enter', preventDefault: () => {} });
      });
    },
    (erroDeLeitura) => {}
  ).catch((err) => {
    toast("⚠️ Permita o acesso à câmara no seu navegador!", true);
    divLeitor.style.display = 'none';
  });
}

function imprimirEtiqueta(idEstoque) {
  let prod = (DB.get(K.e) || []).find(x => x.id === idEstoque);
  if(!prod || !prod.sku) return toast('Este produto não possui código salvo!', true);

  let canvas = document.createElement('canvas');
  try {
    JsBarcode(canvas, prod.sku, { format: "CODE128", width: 2, height: 50, displayValue: true, fontSize: 14 });
  } catch(e) {
    return toast('Aguarde o sistema carregar e tente novamente.', true);
  }
  ...
  w.document.write(`
    ...
    <style>
      ...
      .nome { font-size: 14px; font-weight: bold; margin-bottom: 5px; text-align: center; max-width: 200px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }
      .preco { font-size: 16px; font-weight: bold; margin-top: 5px; }
    </style>
    </head><body>
      <div class="nome">${prod.nome}</div>
      <img src="${barcodeImg}" alt="Código de Barras" style="max-width:100%; height:auto;" />
